...
run_drift() {
  ...
  local targets=(CLAUDE.md RULES.md AGENTS.md skills scripts agents templates plugins prompts)
  local scan=()
  for t in "${targets[@]}"; do [ -e "${REPO_ROOT}/$t" ] && scan+=("${REPO_ROOT}/$t"); done

  # モデル ID らしき文字列だけを狙い撃つ (skill 名 "deepseek-redteam" 等を拾わない)
  local pattern='claude-(opus|sonnet|haiku|fable|mythos)-[0-9][a-zA-Z0-9.-]*|deepseek-(reasoner|chat|v[0-9][a-zA-Z0-9.-]*)|gemini-[0-9][a-zA-Z0-9.-]*|gpt-[0-9][a-zA-Z0-9.-]*|(RedHatAI|Qwen|cyankiwi|unsloth|lmstudio-community|mlx-community|deepseek-ai)/[A-Za-z0-9._-]+'
  ...
  if [ -z "$found" ]; then
    warn "モデル ID が 1 つも見つかりません (パターンが古い可能性)"
    return
  fi
