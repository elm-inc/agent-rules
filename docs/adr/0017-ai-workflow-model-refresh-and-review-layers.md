...
| **床 (LLM)** | 意図と実装の食い違いを独立視点で見る | ローカル LLM (`/local-review`) | **機密案件は必須** / 公開リポは任意 (2026-08-17 改訂) |
...
**決定 1 の修正 (その 1)**: 高リスク変更 (セキュリティ・課金・データ破壊・公開 API・並行処理) では、
**「多様性」層を省略しない**。`/fable-review` は Claude 系であり、`/code-review` も Claude 系なので、
砦だけでは異ベンダーの独立視点がゼロになる。高リスク時は 床 → 多様性 → 深さ → 砦 を全て通す。

## セカンドオピニオン指摘への対応 (`/codex-review`, 2026-08-17)

多様性層 (DeepSeek) の後に、実装視点の異ベンダーとして `codex review --base main` を通した結果:

| 指摘 | 判定 | 対応 |
|---|---|---|
| **CI trigger の `paths` に `AGENTS.md` が無い** — `model-doctor.sh` は走査するのに、そのファイルだけを変更した PR では job が起動せず検知できない | ✅ 妥当 | `paths` を走査対象と一致させ、さらに **doctor 自身に両者の整合チェックを追加** (負のテストで fail することも確認) |
| **probe の secret 判定が `ANTHROPIC_API_KEY` のみ** — DeepSeek/Gemini の secret だけの環境では週次確認が丸ごと skip される。逆に Anthropic だけ設定されていると、**DeepSeek/Gemini が未確認のまま probe が成功する** | ✅ 妥当・重要 | 3 ベンダーいずれかで起動するよう変更。加えて `--strict` を新設し、**1 ベンダーでも未確認なら fail**。R1 の退役は DeepSeek 側で起きたので、部分確認を成功と呼ぶと同じ穴が残る |

> DeepSeek (設計視点) と Codex (実装視点) で**指摘が重複しなかった**。DeepSeek は設計の抜け穴、Codex は CI 配線の具体的欠陥を突いており、異ベンダーを使う意味が実際に出た事例として記録する。
...
- 2026-08-17 採択
- **2026-08-17 改訂 — 床を「機械」と「LLM」に分離し、`/local-review` の起動条件を文脈依存にした**

  当初の決定 1 は床を「無限に回す・機密 OK・0 円 / 常時」と一括で書いていた。
  「0 円なら常時回せばよい」という前提だったが、**仕込みバグ 6 件での実測**
  ([`docs/design/ai-workflow.md`](../design/ai-workflow.md) §4-1) で前提が崩れた:

  | | 検出 | 一意に取れるもの |
  |---|---|---|
  | pre-commit (`ruff --select ALL`) | 3/6 | **リソースリーク** |
  | ローカル LLM | 5/6 | off-by-one・lock 範囲・変数同士の `is` |
  | 和集合 | **6/6** | — |

  - **linter は意味的バグを 1 件も取れない** (全ルールでも 0/3)。LLM 層は代替不能
  - **逆に LLM は「ファイル未 close」を 6/6 で見逃した**。機械層も代替不能
  - よって床は 2 本立てで、**機械層は常時必須**

  一方 LLM 層の「常時」は撤回する。**レビュー層の真のコストは計算資源ではなく読み手の注意**で、
  新モデルは旧モデル比で出力が約 2 倍冗長・説明が粗くなることがあるため、0 円でも注意コストは上がった。
  公開リポで異ベンダーを 1 つ回すなら独立視点として重複するので**省略可**。
  機密案件 (cloud 送信不可) では**唯一の LLM 選択肢なので必須**であり、ここは変更しない。

  なお **`/local-review` が Opus 5 (実装者自身) の見逃しを拾えるか**は未計測で、
  これが床 (LLM) の本当の ROI である。§8 に未測定項目として明記した。
