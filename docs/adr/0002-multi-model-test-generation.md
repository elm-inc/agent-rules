...
採択 (2026-05-24、Phase 1 ベンチマーク完了に伴い確定)
...
- **不採用 (Phase 1b で実測判定済)**: Phase 1b ベンチで 32B INT4 のバグ検出率が **4/5 (-20% 劣化)**、基準 -5% 不達。VRAM 節約も実測 1.5 GiB しかなく ROI 悪い。詳細: [phase7-distill-eval.md](../setup/notes/phase7-distill-eval.md#5-代替案-e-32b-awq-int4-化-は不採用確定)
...
- 2026-05-24 提案
- 2026-05-24 **採択** (Phase 1 ベンチマーク完了に伴い):
  - ロード戦略 **A 案 (swap 方式) で確定**: VRAM 同時ロード不可を実測で確認、B/C 案は不成立
  - **代替案 E (32B INT4 化) 不採用**: 品質劣化 -20% を Phase 1b で確認
  - **DeepSeek-R1-Distill-Qwen-14B online FP8 採用**: API R1 の 60-80% 品質、機密案件用ローカル代替として有用
  - **14B AWQ INT4 採用**: Phase 4 仕込みバグ 5/5 検出、高速ループ + 観点抽出補助に
  - 関連 Linear Project: [テスト工程の多モデル化 + ローカル量子化モデル併用](https://linear.app/elm-inc/project/テスト工程の多モデル化-ローカル量子化モデル併用-e3e9282c4778)
