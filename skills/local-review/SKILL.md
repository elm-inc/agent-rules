...
**pre-commit (ruff/mypy/semgrep) は常時必須で、本スキルはその代替ではない。** 両者は補完関係で、
仕込みバグ 6 件の実測では linter 3/6・本スキル 5/6・**和集合 6/6** だった
(詳細: [`docs/design/ai-workflow.md`](../../docs/design/ai-workflow.md) §4-1)。

| 文脈 | 判断 |
|---|---|
| **機密案件** (cloud へ送信できない) | **必須**。唯一の LLM レビュー選択肢 |
| **公開リポで異ベンダー (`/codex-review` 等) も回す** | **省略可**。独立視点として重複する |
| 公開リポで異ベンダーを回さない | 実行推奨 |

理由: レビュー層の真のコストは計算資源ではなく**読み手の注意**。0 円・オンデマンドでも、
冗長な指摘を読む時間は負債になる。

> **本モデルは指摘箇所は当てるが説明が雑になることがある** (実測: `is` 比較の指摘で場所は正しいが
> 理由が的外れだった)。**説明文をそのまま信じず、場所だけを手掛かりに自分で確認する**こと。
> 逆に「ファイル未 close」のようなリソースリークは 6/6 で見逃したので、そこは linter に任せる。
