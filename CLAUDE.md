...
- **プリンタへ LAN 直送** `/bambu-send` — スライス済み 3mf を FTPS(990)+MQTT(8883) で送って印刷 (クラウド非経由・標準ライブラリのみ)。**安全原則: `print` は `--yes` 必須 (フィラメントを消費する戻せない操作)・実行中のジョブは潰さない・送る前に 3mf の中身を検査し通らなければ送らない・アクセスコードは argv に出さず `~/.config/agent-rules/bambu.toml` (public repo に置かない)・削除サブコマンドを持たない (利用者のファイルを消さない)**
