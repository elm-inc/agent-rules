...
> 💰 **【2026-07-07〜 従量課金】** Fable 5 は利用再開済み。ただし 7/7 以降は **Max クォータ内ではなく実費 (ドル) 課金** になる ($10/$50 per MTok = Opus の 2 倍)。「クォータが余っているから念のため Fable」は成立しない — **価値 (手戻り・正しさ・品質の下流波及) が実費を上回る局面に厳選**し、当月実費を計器で監視する (下記「コスト規律・モニタリング」)。根拠: [`docs/adr/0010`](docs/adr/0010-fable-metered-billing-controls.md)

常用 (セッションモデル) は **Opus 4.8**。Fable 5 は $10/$50 per MTok で Opus の 2 倍 (7/7〜 は実費課金) だが、**価値が高い局面では積極的に活用する**。判断の物差しは「**手戻り・正しさ・品質が下流に大きく波及するか**」。以下を検知したら **Claude が自分で判断して** Fable 5 サブエージェントに委譲する (委譲前に一言宣言し、「なぜ Opus では不足か」を 1 行添える):
...
| **Fable 5 = 両端の専門脳** | 全体設計/分解の起草 (要所) と高リスク変更の最終レビュー (`/fable-task`・`/fable-review` subagent)。7/7〜 実費課金のため厳選 + 計器で監視 (上記コスト規律) |
