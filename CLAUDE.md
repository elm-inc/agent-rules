...
>
> ⚠️ **無人実行では hard cap にする (ADR-0020)**: 上の「ブロックしない人手ゲート」は**人間がその場にいて判断できる場合**の規律。Routines / `/loop` / Workflow の自律ループでは判断者が居ないため**予算超過で停止する**。`bypassPermissions` は「worktree の外に書けない・課金 API を叩かない・本番に触れない」タスクに限る。
>
...
- **並列化は 2 種類あり、規律が違う** (ADR-0020):
  - **協調的分解** (1 タスクを分割・文脈共有) → **最大 5、>10 は無益** (トークン約 7 倍)。探索・調査は Haiku subagent に、フロンティア級 (Fable 5.1 / Astra) は数少ない難所のみ (**安全原則: サブエージェント規律**)
  - **独立競合探索** (同一ゴールに N 個の独立解 → **機械的に選抜**) → **実行可能な採点関数があれば N を上げてよい**。無ければやらない (順位がノイズになり、人間の読む量が N 倍になるだけ)
- **検証層は「数」ではなく「機構の異種性」で強くする。審査 LLM を N 個並べない** — 9 個並べても実効 2 票分にしかならず、最良の単一審査者以下になりうる ([実証](https://arxiv.org/abs/2605.29800))。強化軸は 機械的検査 / 実行による観測 / 変異注入 / 異ベンダー 1 つ
...
- **画像生成・編集** `/nano-banana` — Nano Banana 2 でテキスト→画像 / 参照画像+指示→編集。文字を含む図版は `--pro`。**安全原則: 生成物は既定で `./nano-banana/` に落ちるのでリポ内実行時は `--outdir` でスクラッチへ逃がす (成果物の混入防止)・`-n` は API を N 回呼ぶので料金が枚数比例・機密案件は `GEMINI_API_KEY=` 明示空で中止する (クラウド送信の非常口)**
- **デザインシステム** `/design-system` — 正典 (DTCG トークン) を shadcn registry / Figma / Claude Design / Storybook へ**一方向反映**し、ズレを検査。実装は [`elm-inc/design-system-platform`](https://github.com/elm-inc/design-system-platform) の `dsp` CLI で、スキルは薄い配線 (ADR-0013 の二重ソース回避)。**安全原則: `registry.json` や案件の `globals.css` は生成物なので手で編集しない (正典を直す)・トークンが足りないときに直書きで回避しない (正典に足す)・破壊的なトークン変更の PR には `dsp usage <token>` の横断調査を必ず添える・対象リポ一覧は案件名を含むため public な本リポに置かない (`~/.config/design-system-platform/`)**
...
- **外部送信の検知** `/harness-audit` — 外部 LLM への送信を repo の機密区分 (`.harness.yml` の `sensitivity`) に照らして**送信前に伝える** (遮断しない・未宣言は confidential 扱い)。**安全原則: 外部 LLM に送る処理は必ず送信ヘルパ経由で書く (`curl-secret.sh` / `codex-run.sh` 等。CI のコーパス試験が検査)・区分の宣言は案件名を含むためローカル (`~/.config/agent-rules/harness/`)**。根拠 [`ADR-0023`](docs/adr/0023-harness-declaration-two-channels-egress-detection.md)
