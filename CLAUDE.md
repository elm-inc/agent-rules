...
> ⚠️ **【暫定 2026-06-12〜】Fable 5 は米輸出管理指令で当面利用停止中** (Anthropic が Fable 5 / Mythos 5 を全顧客で無効化、他モデルは影響なし)。`/fable-task`・`/fable-review` は復帰まで **Opus 4.8 (effort `max`/`xhigh` 相当の徹底検証を指示)** で代替し、その旨を明示する。下記の Fable 委譲基準は「Opus 4.8 で最大限丁寧にやる」基準として読み替える。**復帰したらこの注記を消すだけ**で元の Fable 委譲に戻る。
...
| **Fable 5 = 両端の専門脳** (⚠️ 当面停止中→Opus 4.8 代替) | 全体設計/分解の起草 (要所) と高リスク変更の最終レビュー (`/fable-task`・`/fable-review` subagent)。**Fable 5 停止中 (上記注記) は Opus 4.8 で代替** |
